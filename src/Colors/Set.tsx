...
const editContainerColor: SetStateReducer<EditContainerColor> = (prevState, action) => {
  const {value} = action;
...
export const ColorSet: React.SFC<SetProps> = (props) => {
  ...
  const [state, dispatch] = useReducer<SetStateReducer<Action>>(reducer, initialState);
  ...
  return (
    <div style={{display: 'inline-block'}}>
      ...
      <div className="color-set">
        <ColorSlot name='container' color={state.container} onChange={onContainerChange} />
        <div className="color-set__col">
          <ColorSlot name='accent' color={state.accent} ratio={state.accentRatio} onChange={onChange} />
          <ColorSlot name='high' color={state.high} ratio={state.highRatio} onChange={onChange} />
          <ColorSlot name='medium' color={state.medium} ratio={state.mediumRatio} onChange={onChange} />
          <ColorSlot name='low' color={state.low} ratio={state.lowRatio} onChange={onChange} />
        </div>
      </div>
    </div>
  );
